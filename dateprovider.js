...
( function( o ) {
	...
	var mc = function( elem, dtP ) {
		...
		// Generate the text box that will hold the date information
		function composeTextBox( ) {
			...
			if ( ! c.hasOwnProperty("textbox") ) {
				...
				c[ "textbox" ] = document.createElement( "input" );
				c[ "textbox" ].setAttribute( "type", "text" );

				cDiv.appendChild( c["textbox"] );
	
				if ( _dataBind != undefined ) {
					
					c[ "textbox" ].setAttribute( "data-bind", _dataBind );
				}
			}

			x$.bind( c["textbox"], "click",  txtBoxClick );
			x$.bind( c["textbox"], "change",  txtBoxChange );
		}
		...
		function renderMonth( init ) {
			...
			x$.triggerHandler( _that, "renderMonth", true, { days:_widget["content"]["month"]["impl"], weekdays:_widget["content"]["week"]["impl"] } );
		}
		...
		function selectDate( evt ) {
			...
			if( node.tagName === "A" ) {
				...
				var txtBox = elem.childNodes[ 0 ].childNodes[ 0 ];

				txtBox.value =  ( dt.getMonth( )+1 ) + "/" + dt.getDate( ) + "/" + dt.getFullYear( );
